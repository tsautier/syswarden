syswarden_jail_mongodb() {
    ...
    if [[ ! -f "/etc/fail2ban/filter.d/syswarden-mongodb-guard.conf" ]]; then
        cat <<'EOF' >/etc/fail2ban/filter.d/syswarden-mongodb-guard.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/etc/fail2ban/jail.d/syswarden-mongodb.conf
[syswarden-mongodb-guard]
enabled  = true
port     = 27017
filter   = syswarden-mongodb-guard
