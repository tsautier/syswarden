syswarden_jail_wordpress() {
    ...
    if [[ ! -f "/etc/fail2ban/filter.d/syswarden-wordpress-auth.conf" ]]; then
        cat <<'EOF' >/etc/fail2ban/filter.d/syswarden-wordpress-auth.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/etc/fail2ban/jail.d/syswarden-wordpress.conf
[syswarden-wordpress-auth]
enabled  = true
port     = http,https
